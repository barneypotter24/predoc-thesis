...
Prior to Bayesian inference, we estimated unrooted maximum likelihood phylogenetic trees using IQ-TREE \cite{nguyen2015iq}. %GB: mention the version of IQ-TREE
...
\newglossaryentry{bssvs}{name={BSSVS},description={Bayesian stochastic search variable selection}}
...
Beyond the runtime complexity of currently available phylogeographic inference tools, real-time analyses are further impeded by the significant amount of preprocessing that they necessitate before raw sequence data can be analyzed. %GB: check throughout the thesis for the use of 'real time' versus 'real-time' and be consistent
...
While this process is possible to automate for those with sufficient technical knowledge there is no existing method for first-time users of the \gls{beast} framework to do so. %GB: avoid use of 'ecosystem' throughout the thesis
...
For our LASV analyses, we created a Snakemake pipeline similar in form to the one used for preprocessing.
