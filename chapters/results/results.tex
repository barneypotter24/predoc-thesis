...
The initial cause of the lack of success was found to lie within Gaussian Markov Random Field (GMRF) ``block'' transition kernel of the \gls{mcmc} that governed the change in the population size values for the skygrid model. %GB: avoid using 'so-called' which has negative connotation
...
We attempted to rectify this situation by changing to a different transition kernel---a Hamiltonian Monte Carlo operator---however this operator was not able to explore the state space rapidly enough to produce results. %GB: some very negative statements here; you should avoid this as the methods are actually very clever
...
The mean evolutionary rate and times to the most recent common ancestor (\gls{tmrca}) were first estimated for all datasets without including any ancient sequences (and hence not imposing any informative prior information) by collaborators.
...
For genotype A, this leads us to obtain a somewhat increased mean rate of $7.32\times10^{-4}$ [95\% HPD: $4.14\times10^{-4}$--$1.06\times10^{-3}$] substitutions/site/year, while for genotype D we obtained a much decreased mean rate of $9.65\times10^{-5}$ [95\% HPD: $5.97\times10^{-5}$--$1.36\times10^{-4}$] substitutions/site/year. %GB: conclude by saying again that there are no ancient genomes for genotype E
...
Our analysis also revealed that some \gls{hbv}-A sequences with a previously identified subgenotype do not cluster phylogenetically with other members of their subgenotype. %GB: be consistent in using past tense
...
%GB: REMOVE ALL EXCESS WHITESPACE ON THESE FIGURES BELOW!! easily done in Preview on Mac. Also check figure caption for use of past/present tense.
\begin{figure}[ht]
  \centering
  \medskip
...
%GB: check this entire paragraph on use of past and present tense
...
%GB: change 'clock rate' in the figure to 'evolutionary rate'; check carefully again throughout the text and figure captions as well
\begin{figure}[ht]
  \centering
  \medskip
