...
\newglossaryentry{lasv}{name={LASV},description={Lassa virus. The causative agent of Lassa fever, a hemorrhagic fever that affects hundreds of thousands of people anually, primarily in sub-Saharan Africa. One of two viruses being analyzed as part of this thesis, along with HBV}}
...
\newglossaryentry{ctmc}{name={CTMC},description={Continuous-time Markov chain. Markov process in which jumps between states are parameterized in terms of their ``holding time'' in states as a continuous variable, rather than as dicrete-time jump probabilities}}
\newglossaryentry{bssvs}{name={BSSVS},description={Bayesian stochastic serach variable selection. A method by which parameters' inclusion in a model are ``toggled'' randomly to determine whether those parameters should be considered part of the model}}
